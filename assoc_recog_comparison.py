...
def create_model():
    ...
    with model:
        ...
        model.comparison = spa.Compare(D, vocab=vocab_all_words,neurons_per_multiply=200, input_magnitude=.6)
        
        #turns out comparison is not an accumulator - we also need one of those.
        model.comparison_accumulator = spa.State(1, feedback=.1, feedback_synapse=0.1) #fb syn influences speed of acc
        ...
        model.clean_compA = spa.AssociativeMemory(vocab_all_words, wta_output=True, threshold=.3,wta_inhibit_scale=5)
        model.clean_compB = spa.AssociativeMemory(vocab_all_words, wta_output=True, threshold=.3,wta_inhibit_scale=5)
        ...
        def comparison_acc_transform(comparison):
    		comparison_scale = .6
    		comparison_threshold = 0.1 #actually, kind of bias
    		comparison_max = .5
            ...
            spa.Actions(
              #  'motor_input = .04*target_hand',
               # 'dm_learned_words = .1*vis_pair',
                ...
                'clean_compA = vis_pair',
                'clean_compB = representation',
